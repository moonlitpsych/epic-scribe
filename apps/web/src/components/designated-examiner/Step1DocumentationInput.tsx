...
export function Step1DocumentationInput({ initialData, onSubmit, isLoading = false }: Step1Props) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Error Display */}
      {errors.length > 0 && (
        <div className="p-4 bg-[var(--error-bg)] border border-[var(--error-border)] rounded-[2px]">
          <div className="flex items-center gap-2 mb-2">
            <AlertCircle className="text-[var(--error-text)]" size={18} />
            <span className="font-medium text-[var(--error-text)]">Please fix the following:</span>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <div>
            <label className="block text-sm font-medium text-[var(--text-primary)] mb-2">
              Patient Name <span className="text-[var(--error-text)]">*</span>
            </label>
            <input
              type="text"
              value={patientName}
              ...
              className="w-full px-4 py-2 border border-[var(--border-default)] rounded bg-[var(--bg-surface-2)] text-[var(--text-primary)] placeholder:text-[var(--text-muted)] focus:ring-2 focus:ring-[var(--accent-warm)] focus:border-transparent"
            />
          </div>
      ...
      <div className="bg-[var(--bg-surface)] rounded-[2px] border border-[var(--border-default)] p-6">
        <div className="flex items-center gap-2 mb-2">
          ...
          <h3 className="text-lg font-semibold text-[var(--text-primary)]">
            Comprehensive Psychiatric Evaluation (CDE) <span className="text-[var(--error-text)]">*</span>
          </h3>
        </div>
        ...
        <textarea
          ...
          rows={14}
          className="w-full px-4 py-3 border border-[var(--border-default)] rounded bg-[var(--bg-surface-2)] text-[var(--text-primary)] placeholder:text-[var(--text-muted)] focus:ring-2 focus:ring-[var(--accent-warm)] focus:border-transparent font-mono text-sm resize-none"
        />
        <div className="flex items-center justify-between mt-2">
          <p className="text-sm text-[var(--text-secondary)]">{cdeWordCount} words</p>
          {!cdeNote.trim() && (
            <p className="text-sm text-[var(--error-text)]">Required</p>
          )}
        </div>
      </div>
      ...
      <div className="bg-[var(--bg-surface)] rounded-[2px] border border-[var(--border-default)] p-6">
        <div className="flex items-center gap-2 mb-2">
          <FileText className="text-[var(--text-secondary)]" size={20} />
          <h3 className="text-lg font-semibold text-[var(--text-primary)]">
            Progress Notes
            <span className="ml-2 text-sm font-normal text-[var(--text-secondary)]">(Optional but encouraged)</span>
          </h3>
        </div>
        <p className="text-sm text-[var(--text-secondary)] mb-4">
        ...
        <textarea
          ...
          className="w-full px-4 py-3 border border-[var(--border-default)] rounded bg-[var(--bg-surface-2)] text-[var(--text-primary)] placeholder:text-[var(--text-muted)] focus:ring-2 focus:ring-[var(--accent-warm)] focus:border-transparent font-mono text-sm resize-none"
        />
        <p className="text-sm text-[var(--text-secondary)] mt-2">{progressWordCount} words</p>
      </div>
      ...
      <div className="bg-[var(--bg-surface)] rounded-[2px] border border-[var(--border-default)] p-6">
        <div className="flex items-center gap-2 mb-2">
          ...
          <h3 className="text-lg font-semibold text-[var(--text-primary)]">
            Ad-hoc Notes
            <span className="ml-2 text-sm font-normal text-[var(--text-secondary)]">(Optional)</span>
          </h3>
        </div>
        <p className="text-sm text-[var(--text-secondary)] mb-4">
      ...
      <button
        ...
        className="w-full flex items-center justify-center gap-2 px-6 py-4 bg-[var(--accent-primary)] text-[var(--text-inverse)] rounded hover:bg-[var(--accent-primary-hover)] transition-colors disabled:opacity-50 disabled:cursor-not-allowed text-lg font-medium"
      >
        {isLoading ? (
          <>
